	...
	public class Cell : MonoBehaviour, IDropHandler
	{
				...
				if (myDadItem == item) // If item dragged from this cell
				{
					...
					switch (cellType)
					{
						case CellType.DropOnly:
							Item.itemDragging.SetActive(false); // Item can not be dragged. Hide icon
							break;
					}
				}
			}
		...
		private void PlaceItem(Item item)
		{
			if (item != null)
			{
				...
				myDadItem = null;
				Cell cell = item.GetComponentInParent<Cell>();
				if (cell != null)
				{
		...
		public void SwapItems(Cell firstCell, Cell secondCell)
		{
			...
			{
				//Item firstItem = firstCell.GetItem(); // Get item from first cell
				Item firstItem =Item.itemDragging.GetComponent<Item>();
				Item secondItem = secondCell.GetItem(); // Get item from second cell
				// Swap items
				if (firstItem != null)
				{
